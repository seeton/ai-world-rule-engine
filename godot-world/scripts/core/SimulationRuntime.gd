	...
	return {
		...
		"entities": {
			"player_character": {
				...
				"archetype": "player",
				"tags": ["player", "character", "mortal", "mutable"],
				"position": {
					"x": 0.0,
					"y": 0.9,
					"z": 0.0,
					"location": "origin_clearing"
				},
				"render_3d": {
					"kind": "character",
					"size": {
			...
			"game_master": {
				...
				"name": _text("gm_name"),
				"archetype": "gm",
				"tags": ["gm", "character", "immortal", "immutable"],
				"position": {
					"x": -2.5,
					"y": 1.1,
					"z": -1.5,
...
func _resolve_parent_rule_links(rule: Dictionary, installed_rules: Dictionary) -> Dictionary:
	...
	if not missing_rule_kinds.is_empty():
		return {
			"status": "error",
			"message": "Rule '%s' requires installed parent rule kinds [%s]. Install a compatible rule that provides these capabilities first." % [rule_id, ", ".join(missing_rule_kinds)],
			"rule_id": rule_id,
...
func _apply_gravity(entities: Dictionary, step_seconds: float) -> void:
	...
	for entity_id in entity_ids:
		...
		if is_grounded and not was_grounded:
			_append_event("gravity_landed", "'%s' landed on the preview floor." % String(entity.get("name", entity_id)), {"entity_id": entity_id, "floor_y": target_floor_y})
