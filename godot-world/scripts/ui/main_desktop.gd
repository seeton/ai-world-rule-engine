...
func _build_poc4_admin_panel() -> Control:
    var panel := _make_panel_section("PoC4 proposal / 管理状態", "pending proposal、review 状態、apply 結果、backend error を read-only で確認します。")
    var body := panel.get_meta("body") as VBoxContainer
...
func _update_status_label() -> void:
    ...
    _status_label.text = "GM会話データ元: %s | 候補テンプレート: %d | 稼働ルール: %d | PoC4: %s" % [source, _template_cache.size(), _installed_rule_cache.size(), poc4_status]
