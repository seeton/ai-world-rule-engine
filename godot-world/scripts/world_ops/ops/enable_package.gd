extends RefCounted
class_name WorldOpEnablePackage
...
static func validate(world: Object, request: Dictionary) -> Dictionary:
	...
	if package_id.is_empty():
		errors.append("EnablePackage.package_id must be a non-empty string.")
	elif world != null:
		...
		elif bool(package_summary.get("all_rules_enabled", package_summary.get("enabled", false))):
			warnings.append("package '%s' is already enabled." % package_id)
...
static func dry_run(world: Object, request: Dictionary) -> Dictionary:
	...
	lines.append("[dry-run] would set package '%s' enabled=true (was %s)." % [package_id, previous_state])
	var preview_diff := _build_diff(package_id, previous_state, "enabled")
	return WorldOpResultScript.dry_run(TYPE, lines, {"package_id": package_id, "would_set_enabled": true, "previous_state": previous_state}, preview_diff, _rollback_hint(package_id))
...
static func execute(world: Object, request: Dictionary) -> Dictionary:
	...
	var action_result: Dictionary = CliActionsScript.set_package_enabled(world, package_id, true)
	var status := String(action_result.get("status", ""))
	if status == "enabled":
		var lines := PackedStringArray()
		lines.append("package '%s' enabled (was %s)." % [package_id, previous_state])
		var diff := _build_diff(package_id, previous_state, "enabled")
		var payload := {
		...
		return WorldOpResultScript.ok(TYPE, lines, payload, diff, _rollback_hint(package_id))
	var error_lines := PackedStringArray()
	error_lines.append("EnablePackage failed: %s" % String(action_result.get("message", status)))
...
static func _rollback_hint(package_id: String) -> Dictionary:
	return {"supported": true, "hint": "Inverse op: DisablePackage package_id=%s" % package_id}
