...
func _initialize() -> void:
	...
	if exit_code == 0:
		var event_install_result: Dictionary = world.create_rule_from_patch({
			"id": "cli_smoke_event_rule",
			"name": "CLI Smoke Event Rule",
			"enabled": true,
			"metadata": {"package_id": "cli.smoke"},
			"install_actions": [{
				...
				"value": {
					"cli.smoke": {
						"enabled": true,
						"event_bindings": [{"event": "ping", "target_rule": "cli_smoke_event_rule"}],
						"relations": [],
						"rule_operations": [{
							"rule_id": "cli_smoke_event_rule",
							"rule_type": "event_visual_effect",
							"effects": [{"effect_type": "spawn_visual", "tag": "cli_smoke", "value": "spark"}]
						}]
					}
				}
			}]
		})
		if String(event_install_result.get("status", "")) != "installed":
			exit_code = 1
			failure_message = "Failed to install event smoke rule: %s" % JSON.stringify(event_install_result)
	if exit_code == 0:
		...
		var baseline_event_result: Dictionary = world.dispatch_input_event("ping", {})
		if int(baseline_event_result.get("triggered_effect_count", 0)) <= 0:
			exit_code = 1
			failure_message = "Event runtime did not trigger before package disable: %s" % JSON.stringify(baseline_event_result)
		...
		if exit_code == 0:
			...
			if String(disable_result.get("status", "")) != "disabled":
				...
				if not _rule_has_enabled(snapshot_after_disable, "cli_smoke_rule", false):
					...
					var disabled_event_result: Dictionary = world.dispatch_input_event("ping", {})
					if String(disabled_event_result.get("status", "")) != "ignored" or int(disabled_event_result.get("triggered_effect_count", -1)) != 0:
						exit_code = 1
						failure_message = "Event runtime stayed active after package disable: %s" % JSON.stringify(disabled_event_result)
					else:
						...
						if abs(hunger_after_disable - baseline_hunger) > 0.0001:
							...
							if String(save_result.get("status", "")) != "saved":
								...
								if String(load_result.get("status", "")) != "loaded":
									...
									var disabled_event_after_load: Dictionary = loaded_world.dispatch_input_event("ping", {})
									if String(disabled_event_after_load.get("status", "")) != "ignored":
										exit_code = 1
										failure_message = "Loaded world reactivated disabled package event runtime: %s" % JSON.stringify(disabled_event_after_load)
									else:
										...
										if String(enable_result.get("status", "")) != "enabled":
											...
											var reenabled_event_result: Dictionary = loaded_world.dispatch_input_event("ping", {})
											if int(reenabled_event_result.get("triggered_effect_count", 0)) <= 0:
												exit_code = 1
												failure_message = "Event runtime did not resume after package re-enable: %s" % JSON.stringify(reenabled_event_result)
											else:
