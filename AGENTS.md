...
- 文書、コード、設定変更を作成または更新した場合、その変更がユーザ側の実証を必要としないなら、ローカル編集で止めず **commit -> push -> PR 作成** まで同一セッションで進めること。
- ここでいう「ユーザ側の実証」には、実機確認、プレイ確認、ユーザ判断を伴う挙動確認、またはユーザにしかできない確認作業を含む。
- 逆に、ユーザ側の実証が必要な変更では、そこで必要な確認内容を明示したうえで手前で止めてよい。
