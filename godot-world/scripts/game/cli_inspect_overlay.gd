...
# Renders a CLI-style scrollback + LineEdit input. Same command syntax as
# the headless CLI (scripts/cli/main.gd); both go through
# scripts/cli/cli_command_parser.gd so they cannot drift.
...
func _unhandled_input(event: InputEvent) -> void:
	...
	# Only allow the C key to close the overlay when the LineEdit doesn't
	# already own focus — otherwise typing 'c' inside a command would close.
...
func _build_header(layout: VBoxContainer) -> void:
	...
	description.text = "ゲーム内 CLI。headless CLI と同じ文法 (inspect / rule / package / snapshot / help / clear) を打てます。actuation はすべて engine-safe API を経由します。"
	...
	help_label.text = "C / Esc: 閉じる / ↑↓: ヒストリ"
...
func _build_input(layout: VBoxContainer) -> void:
	...
	_input.placeholder_text = "コマンドを入力 (例: inspect, rule disable rule_id, snapshot dump user://world.json)"
	_input.text_submitted.connect(_on_input_submitted)
	_input.gui_input.connect(_on_input_gui_input)
	row.add_child(_input)

	var submit := Button.new()
	submit.text = "送信 (Enter)"
	submit.pressed.connect(_on_submit_pressed)
	row.add_child(submit)

	# Defer focus so the LineEdit owns input from frame 1.
...
func _on_input_submitted(text: String) -> void:
	...
	if args.is_empty():
		return
	var result: Dictionary = CliCommandParserScript.dispatch(_world_state, args, {})
	...
	if String(result.get("status", "")) == "directive":
		...
		match String(payload.get("directive", "")):
			"clear":
				...
				return
		# Unknown directive — fall through and just print lines.

	var status := String(result.get("status", ""))
	var color := COLOR_OUTPUT
	if status == "ok":
		color = COLOR_OK
	elif status == "error" or status == "usage_error":
		color = COLOR_ERROR
...
func _print_intro() -> void:
	_append_hint("World CLI へようこそ。'help' で一覧、'clear' でスクロールバックを消去、C / Esc で閉じます。")
	if _auto_open_reasons.size() > 0:
		_append_colored("(自動オープン: %s)" % ", ".join(_auto_open_reasons), COLOR_ERROR)
	_handle_result(CliCommandParserScript.dispatch(_world_state, PackedStringArray(["help"]), {}))
...
func _tokenize(line: String) -> PackedStringArray:
	# Minimal shlex-like tokenizer: split on whitespace, support double quotes.
