...
static func dry_run(world: Object, request: Dictionary) -> Dictionary:
	...
	return WorldOpResultScript.dry_run(TYPE, lines, {"package_id": package_id, "would_set_enabled": true, "previous_state": previous_state}, preview_diff, _rollback_hint(package_id, previous_state))
...
static func _rollback_hint(package_id: String, previous_state: String) -> Dictionary:
	...
	return {
		"supported": false,
		"hint": "Mixed/already-enabled package states require manual per-rule rollback for package_id=%s." % package_id
	}
