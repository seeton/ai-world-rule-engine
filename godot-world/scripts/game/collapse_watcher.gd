...
# auto-open the CLI inspect overlay so the player has an immediate entry
...
# - The watcher does no UI work itself; cooldown/exclusivity logic lives in
#   the consumer (game_scene.gd) so the watcher stays trivially testable.
...
# Poll once on demand. Exposed for tests and for callers that want to
# trigger an immediate check (e.g. after closing the GM screen).
func poll_once() -> void:
...
# Reset the watcher's baseline so the next poll is treated as fresh. Used
# when the consumer wants to re-arm after a known transition (e.g. after
# the player resolved the issue).
func reset_baseline() -> void:
	_baseline_taken = false
	_previous_signals = {}


# Inject a WorldState (test hook). Production code uses /root/WorldState.
