...
usage() {
  cat <<'EOF'
Usage:
  scripts/launch_godot.sh <issue-number> [--project <path>] [--allow-detached-head] [--dry-run] [-- <godot-args...>]

Launch the Godot project from a repo-local issue worktree instead of the repository root.
The launcher resolves godot-world inside .agent-workspaces/issue-<number>/ and refuses repo-root paths or paths outside that worktree.

Options:
  --project <path>         Godot project directory inside the issue worktree (default: godot-world)
  --allow-detached-head    Permit launching from a detached-HEAD worktree when that is intentional
  --dry-run                Validate and print the launch command without starting Godot
  --                       Pass remaining arguments directly to Godot
EOF
}
...
worktree_exists "${target_worktree}" || die "No registered worktree at ${target_worktree}. Create or reuse it before launching Godot."
...
launch_command=("${godot_bin}" --path "${resolved_project}")
if [[ "${#godot_args[@]}" -gt 0 ]]; then
  launch_command+=("${godot_args[@]}")
fi
