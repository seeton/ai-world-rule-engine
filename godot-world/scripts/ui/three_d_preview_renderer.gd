...
func _ready() -> void:
    ...
    _build_ui()
    _show_placeholder(
        "3D preview is waiting for snapshot[\"three_d_preview\"] data from WorldState.\nAsk for a 3D world view to populate this panel."
    )
...
func update_from_snapshot(snapshot: Dictionary) -> void:
    ...
    if not (preview_value is Dictionary):
        _show_placeholder(
            "3D preview is unavailable in this snapshot.\nWorldState can expose snapshot[\"three_d_preview\"] with renderables, lighting, gravity, and camera hints."
        )
        return
    ...
    if not _variant_to_bool(preview_data.get("enabled", false)):
        _show_placeholder(
            "3D preview exists but is disabled.\nSet snapshot[\"three_d_preview\"][\"enabled\"] = true when the user requests a 3D view."
        )
        return
...
func _show_placeholder(message: String) -> void:
    ...
    _status_label.text = "3D preview inactive"
...
func _render_preview(preview_data: Dictionary) -> void:
    ...
    _status_label.text = "3D preview active | %d renderables | light %s | shadows %s | gravity %s" % [
        normalized_renderables.size(),
        "on" if lighting_enabled else "off",
        "on" if shadows_enabled else "off",
        "on" if gravity_enabled else "off"
    ]
