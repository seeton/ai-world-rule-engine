...
# Shared "inspect" report builder used by both the headless CLI dispatcher
# (scripts/cli/main.gd) and the in-game C-key overlay
# (scripts/game/cli_inspect_overlay.gd). Keep all field names / collapse
# signals in one place so the overlay genuinely mirrors what the CLI sees.
